...
El requerimiento principal de la bomba es mantener el caudal real lo más cercano posible al caudal indicado por la orden médica. En la Figura 4.1 se observa la evolución de ambas señales.

\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.9\textwidth]{images/plot_caudales.png}
    \caption{Evolución temporal del caudal objetivo versus el caudal sensado por el dispositivo. La franja sombreada representa el margen de tolerancia operativa del $\pm 10\%$. Las líneas verticales verdes indican el ingreso de nuevas órdenes médicas.}
    \label{fig:caudales}
\end{figure}

Como se aprecia en la gráfica, el caudal real persigue el objetivo de forma estable. Los picos que exceden la franja sombreada corresponden a perturbaciones físicas inyectadas (ruido del sensor), las cuales el sistema debe procesar para determinar si constituyen una falla crítica o una anomalía pasajera.
...
Se verifica visualmente el cumplimiento estricto de las reglas temporales: el integrador acumula segundos únicamente cuando el caudal sale de la banda del $10\%$. Al interceptar el umbral de 5 segundos, se dispara lógicamente el nivel de alerta correspondiente.
...
La distribución demuestra que el diseño prioriza operar de forma preventiva (abundancia de alarmas BAJA o MEDIA), reservando el estado CRÍTICO exclusivamente para escenarios donde la falla es inminente o el personal no responde a los llamados de atención.
...
Esta gráfica final satisface el requerimiento de observar el estado general de la máquina a lo largo del tiempo. Facilita la interpretación clínica al aislar el ruido puro del sensor y enfocarse categóricamente en la condición de seguridad que experimenta el paciente en cada instante $t$.