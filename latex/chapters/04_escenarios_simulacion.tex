...
\begin{figure}[hbt!]
    ...
    \caption{Evolución temporal del caudal de la bomba. Se compara el caudal objetivo prescrito, el caudal real entregado al paciente y las lecturas ruidosas individuales del sensor. La franja sombreada representa el margen de tolerancia operativa del $\pm 10\%$. Las líneas verticales verdes indican el ingreso de nuevas órdenes médicas.}
    \label{fig:caudales}
\end{figure}

En este gráfico se distinguen tres elementos fundamentales:
\begin{itemize}
    ...
    \item \textbf{Caudal Real (Entrega Física al Paciente) - Línea roja:} Es el caudal físico real de líquido entregado por el actuador de la bomba. Se observa que persigue fielmente al caudal objetivo con un pequeño retardo mecánico transitorio ($0$ a $0.5$ segundos). Ambas líneas se superponen formando una línea violeta durante la operación normal.
    \item \textbf{Lecturas del Sensor (Señal con Ruido) - Puntos naranjas:} Representa las muestras discretas de caudal medido que el sensor de flujo envía al controlador cada 1 segundo. Estas muestras incorporan el ruido gaussiano del 20\%, distribuyéndose como una nube de puntos alrededor del caudal real. Es la única información que el controlador posee para evaluar el estado de la infusión.
\end{itemize}

\textbf{Análisis de Observaciones Clave:}
\begin{itemize}
    \item \textbf{Detención por Desvío Crítico (alrededor del segundo 30/100):} Se puede observar cómo la nube de puntos naranjas (lecturas del sensor) se dispersa consistentemente fuera de la franja azul de tolerancia del $\pm 10\%$. Debido a que el desvío se sostiene por 10 segundos continuos, el controlador detecta una falla crítica y detiene preventivamente la bomba. En el gráfico, esto se evidencia cuando la línea roja (caudal real) cae abruptamente a $0~ml/h$, mientras que la línea azul (caudal objetivo prescrito) permanece en su valor original. Esto genera una clara discrepancia visual que representa la parada de seguridad.
    \item \textbf{Detención por Fin de Bolsa (alrededor del segundo 800 a 1100):} A medida que el líquido es infundido, el volumen de la bolsa decrece hasta agotarse. En la gráfica se aprecia el instante exacto en que la bolsa se queda sin líquido (alrededor del segundo 800-1100 dependiendo de la semilla y los caudales aleatorios previos). Al agotarse el líquido y transcurrir la ventana de gracia de 60 segundos sin confirmación de recarga, la bomba detiene la entrega por seguridad, haciendo que la línea roja caiga a $0~ml/h$ a pesar de que el objetivo de la orden (línea azul) siga activo.
\end{itemize}
...
A partir del análisis combinado de las métricas anteriores, se abstrajo el comportamiento de la bomba en tres macro-estados operativos. La Figura 4.4 ilustra este ciclo de vida.
...
Esta gráfica final categoriza el estado de la bomba de infusión en tres condiciones lógicas muy claras:
\begin{itemize}
    \item \textbf{Normal (Estado 1):} La bomba se encuentra infundiendo líquido activamente y las lecturas medidas por el sensor se encuentran dentro de la banda de tolerancia del $\pm 10\%$. Representa una entrega segura y estable.
    \item \textbf{Anomalía (Estado 2):} La bomba continúa infundiendo líquido, pero la lectura del sensor de flujo ha caído fuera de la banda del $\pm 10\%$. Este estado es transitorio: si la lectura reingresa a la banda de tolerancia, el sistema retorna a \textit{Normal}; si permanece en anomalía por 10 segundos, escala a una detención de seguridad.
...
\textbf{Análisis de Observaciones Clave:}
\begin{itemize}
    \item Se puede constatar que la bomba oscila constantemente entre los estados \textit{Normal} y \textit{Anomalía} debido al ruido inherente de la medición. Sin embargo, las caídas sostenidas al estado \textit{Detenida} (Estado 0) ocurren únicamente cuando expira la bolsa (segundo 800-1100) o ante una alarma crítica de desvío (segundo 30/100), alineándose perfectamente con las caídas a cero de la línea roja en el gráfico de caudales.
\end{itemize}